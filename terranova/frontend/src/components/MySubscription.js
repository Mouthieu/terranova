...
const MySubscription = ({ composter, user_id }) => {
    const [deleted, setDeleted] = React.useState(false)
    const deleteSubscription = (user_id, composter_id) => {
        axios.post(`http://127.0.0.1:8000/api/unsubscribe/${user_id}/${composter_id}/`)
        .then(response => {
    ...
    return (
        <div className="subscription-card" key={composter.id}>
            ...
            <button className="delete-icon-button" onClick={() => deleteSubscription(user_id, composter.id)}>
